...
def expert_max_violation_batchwise(
    ...
    effective_total_tokens = tokens_per_expert.sum()
...
def compute_expert_metrics(
    ...
    avg_ffn_expert_usage_per_token = ffn_experts_per_token.to(torch.float32).mean()
    std_ffn_expert_usage_per_token = ffn_experts_per_token.to(torch.float32).std()

    return total_zero_expert_tokens, tokens_with_only_zero_experts, total_ffn_expert_tokens, tokens_with_only_ffn_experts, avg_ffn_to_zero_expert_ratio_per_token, avg_ffn_expert_usage_per_token, std_ffn_expert_usage_per_token
